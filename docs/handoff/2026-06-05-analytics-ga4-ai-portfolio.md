...
- `src/utils/analytics.ts`
  ...
  - 커스텀 이벤트는 GTM `dataLayer`로 push한다.
  - GA4 수집은 GTM 컨테이너 안에서 태그와 트리거로 관리한다.
...
- `src/app/layout.tsx`
  ...
  - `GTM_ID` 환경변수가 있으면 Google Tag Manager 태그를 삽입한다.
  - `GTAG_ID` 직접 삽입은 중복 집계 방지를 위해 사용하지 않는다.
...
GA4 이벤트는 GTM 중심으로 관리한다. Vercel 환경변수에는 아래 값만 둔다.

```text
GTM_ID=GTM-...
```

`GTAG_ID=G-...`는 앱에서 직접 읽지 않는다. GA4 측정 ID는 GTM 컨테이너 안의 GA4 태그에서 관리한다.
