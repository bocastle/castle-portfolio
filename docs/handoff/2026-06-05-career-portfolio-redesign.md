...
- AI 협업 포트폴리오 개선 프로젝트를 진행형 대표 프로젝트로 추가했고, private 프로젝트도 공개 가능한 구조 설명 중심으로 정리했다.
...
- `docs/project-briefs/castlecms-project-brief.md`를 추가해 private `castleCms` 프로젝트를 공개 가능한 구조 설명 자료로 정리했다.
- `docs/project-briefs/castlecms-api-brief.md`를 추가해 비밀값 없는 API 목록, 요청/응답 예시, 오류 응답 기준, 검증 포인트를 정리했다.
- 문서에는 문제 의식, Frontend/Backend 구조, 관리자 Bearer token 인증과 외부 API Key 인증 분리, Mermaid 권한 흐름, 검증 범위를 포함했다.
- `src/components/Projects/data.json`의 `castleCms` 설명을 프로젝트 설명 자료와 맞춰 운영형 CMS 설계/검증 사례로 더 짧게 다듬었다.
- 프로젝트 카드에 GitHub 문서 링크를 추가해 `castleCms`의 프로젝트 요약과 API 설계 요약을 공개 화면에서 열 수 있게 했다.
- `BuildLog`에 `castleCms 자료 정리` 항목을 추가해 요구사항 정리, 문서화, 테스트, 배포 검증 흐름을 포트폴리오 개선 기록으로 누적했다.
...
- GitHub 문서 URL `docs/project-briefs/castlecms-api-brief.md`는 HTTP 200으로 접근 가능하다.
- `https://bocelog.vercel.app/` 홈은 HTTP 200으로 접근 가능하다.
- 확인 시점의 공개 홈 HTML에는 `castleCms 자료 정리`, `API 설계 요약`, `castlecms-api-brief.md`가 아직 포함되지 않았다. Vercel 배포 또는 prerender 반영 지연 가능성이 있으므로 잠시 뒤 다시 확인한다.
